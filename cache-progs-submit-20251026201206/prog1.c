...
// 24 KB worth of ints
#define ARRAY_SIZE (24 * 1024 / sizeof(int))
// 64 bytes per step      
#define STEP 16            

int arr[ARRAY_SIZE] __attribute__((aligned(64)));
...
#pragma clang loop interleave(disable)
    ...
    // Initialize the array so each element points to another 64B block
    for (int i = 0; i < ARRAY_SIZE; ++i) {
        global_array[i] = (i + STEP) % ARRAY_SIZE;
    }
