...
// need to define variables pre runtime for this test
...
// 16 bytes step
#define STEP 16
// Number of blocks to cycle through  
...
int main() {
    ...
    static int arr[N_BLOCKS * BLOCK_SIZE];

    // create a cyclic pointer chain spaced by step size
    for (int i = 0; i < N_BLOCKS; i++) {
        arr[i * STEP] = ((i + 1) % N_BLOCKS) * STEP;
    }
